...
namespace VVVV.Nodes
{
	...
	public class ValueHeartbeatNode : IPluginEvaluate
	{
		...
		public IDiffSpread<int> FFramerate;
		...
		public void Evaluate(int SpreadMax)
		{
			...
			if (FFramerate.IsChanged && FFramerate.SliceCount > 0 ) {
				int _frameRate = FFramerate[0];
				frameRate = _frameRate >= 0 ? (uint)_frameRate : 0;
				frameRate = (uint)(1000/(float)frameRate);
			}
			...
			if(FInput.IsChanged)
			{
				for (int i = 0; i < SpreadMax; i++) {
					...
					sampleCounter += frameRate;                         // keep track of the time in mS with this variable
					Signal = FInput[i];
